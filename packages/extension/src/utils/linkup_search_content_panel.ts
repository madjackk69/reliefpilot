// Webview panel to display the static markdown content produced by Linkup tools.
import * as vscode from 'vscode'
import { env } from './env'
import { getLinkupContentSession } from './linkup_search_content_sessions'

export async function openLinkupContentPanelByUid(uid: string): Promise<void> {
    let session = getLinkupContentSession(uid)
    if (!session) {
        const deadline = Date.now() + 30000
        while (!session && Date.now() < deadline) {
            await new Promise(r => setTimeout(r, 150))
            session = getLinkupContentSession(uid)
        }
        if (!session) {
            return
        }
    ...
    const panel = vscode.window.createWebviewPanel(
        'reliefpilot.linkupContent',
        'Relief Pilot: Linkup Content',
...
<html lang="en">
  <head>
    ...
    <title>Relief Pilot: Linkup Content</title>
    <style>
  ...
  <body>
    <div class="layout">
      <div class="col">
        <p class="section-title">Linkup Tool Output (Markdown)</p>
