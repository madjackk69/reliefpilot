// Webview panel to display the static markdown content produced by Exa tools.
import * as vscode from 'vscode'
import { env } from './env'
import { getExaContentSession } from './exa_search_content_sessions'

export async function openExaContentPanelByUid(uid: string): Promise<void> {
    let session = getExaContentSession(uid)
    if (!session) {
        const deadline = Date.now() + 30000
        while (!session && Date.now() < deadline) {
            await new Promise(r => setTimeout(r, 150))
            session = getExaContentSession(uid)
        }
        if (!session) {
            return
        }
    ...
    const panel = vscode.window.createWebviewPanel(
        'reliefpilot.exaContent',
        'Relief Pilot: Exa Content',
...
<html lang="en">
  <head>
    ...
    <title>Relief Pilot: Exa Content</title>
    <style>
  ...
  <body>
    <div class="layout">
      <div class="col">
        <p class="section-title">Exa Tool Output (Markdown)</p>
